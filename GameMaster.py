...
class GameMaster(Usuario):
    ...
    def ListarPoderesPersonaje(self):
        ...
        SeleccionPersonaje = PersonajesEnPartida[EleccionPersonaje]
        Idpersonaje = SeleccionPersonaje[0]
        QueryPoderes = f"""select poder_personaje.id, poder.id, poder.nombre
        from poder_personaje
        inner join poder on poder.id = poder_personaje.fk_id_poder
        where poder_personaje.fk_id_personaje = {Idpersonaje}
    ...
    def ListarHabilidadesPersonaje(self):
        ...
        SeleccionPersonaje = (PersonajesEnPartida[EleccionPersonaje])
        print(int(SeleccionPersonaje[4]))
        IdPersonaje = int(SeleccionPersonaje[0])
        ...
        where habilidad_personaje.fk_id_personaje = {IdPersonaje}
    ...
    def CambiarHabilidadPersonaje(self):
        DatosPersonaje = self.ListarHabilidadesPersonaje()
        if DatosPersonaje == False:
            return
        EleccionCambioHabilidad = int(input("¿Qué habilidad desea de cambiar?")) -1
        SeleccionCambioHabilidad = DatosPersonaje[0][EleccionCambioHabilidad]
        id_habilidad_Seleccionada = SeleccionCambioHabilidad[0]
        NombreHabilidad = SeleccionCambioHabilidad[2]
        print(f"Se reemplazara la habilidad {NombreHabilidad}")
        if EleccionCambioHabilidad in [0, 1]:
            print("Las habilidades equipadas por el jugador no pueden ser cambiadas")
            return
        Raza = DatosPersonaje[1][4]
        print("Puede reemplazar la habilidad anterior por una de las siguientes habilidades: ")
        HabilidadesDisponibles = self.ListarHabilidades(Raza)
        EleccionHabilidadNueva = int(input("Seleccione (numero) cual habilidad reemplazara a la habilidad anterior")) -1
        SeleccionHabilidadNueva = HabilidadesDisponibles[EleccionHabilidadNueva][0]
        QueryCambioDeHabilidad = f"""UPDATE habilidad_personaje
        set fk_id_habilidad = {SeleccionHabilidadNueva}
        where id = {id_habilidad_Seleccionada}
        """
        self.cursor.execute(QueryCambioDeHabilidad)
        print("Se ha cambiado la habilidad exitosamente")

    def eliminarhabilidadPersonaje(self):
        DatosPersonaje = self.ListarHabilidadesPersonaje()
        if DatosPersonaje == False:
            return
        EleccionEliminacionHabilidad = int(input("¿Qué habilidad desea de eliminar?")) -1
        SeleccionEliminacionHabilidad = DatosPersonaje[0][EleccionEliminacionHabilidad]
        id_habilidad_Seleccionada = SeleccionEliminacionHabilidad[0]
        NombreHabilidad = SeleccionEliminacionHabilidad[2]
        print(f"Se eliminara la habilidad {NombreHabilidad}")
        if EleccionEliminacionHabilidad in [0, 1]:
            print("Las habilidades equipadas por el jugador no pueden ser eliminadas")
            return
        QueryEliminarHabilidad = f"""DELETE from habilidad_personaje 
        where id = {id_habilidad_Seleccionada}"""
        self.cursor.execute(QueryEliminarHabilidad)
    ...
    def AgregarPoderApersonaje(self):
        ...
        AgregarHabilidadAPjQuery = f"""INSERT INTO poder_personaje (fk_id_poder, fk_id_personaje)
    ...
    def CambiarPoderPersonaje(self):
        DatosPersonaje = self.ListarPoderesPersonaje()
        if DatosPersonaje == False:
            return
        EleccionCambioPoder = int(input("¿Qué poder desea de cambiar?")) -1
        SeleccionCambioPoder = DatosPersonaje[0][EleccionCambioPoder]
        id_poder_seleccionado = SeleccionCambioPoder[0]
        NombrePoder = SeleccionCambioPoder[2]
        print(f"Se reemplazara el poder {NombrePoder}")
        if EleccionCambioPoder in [0]:
            print("Los poderes equipados por el jugador no pueden ser cambiados")
            return
        Raza = DatosPersonaje[1][4]
        print("Puede cambiar el poder anterior por uno de los siguientes poderes: ")
        PoderesDisponibles = self.ListarPoderes(Raza)
        EleccionNuevoPoder = int(input("Seleccione (numero) cual poder reemplazara al poder anterior")) -1
        SeleccionPoderNuevo = PoderesDisponibles[EleccionNuevoPoder][0]
        QueryCambioPoder = f"""UPDATE poder_personaje
        set fk_id_poder = {SeleccionPoderNuevo}
        where id = {id_poder_seleccionado}
        """
        self.cursor.execute(QueryCambioPoder)
        print("Se ha cambiado el poder exitosamente")

    def eliminarPoderPersonaje(self):
        DatosPersonaje = self.ListarPoderesPersonaje()
        if DatosPersonaje == False:
            return
        EleccionEliminacionPoder = int(input("¿Qué habilidad desea de eliminar?")) -1
        SeleccionEliminacionPoder = DatosPersonaje[0][EleccionEliminacionPoder]
        id_poder_seleccionado = SeleccionEliminacionPoder[0]
        NombrePoder = SeleccionEliminacionPoder[2]
        print(f"Se eliminara el poder {NombrePoder}")
        if EleccionEliminacionPoder in [0]:
            print("Las habilidades equipadas por el jugador no pueden ser eliminadas")
            return
        QueryEliminarPoder = f"""DELETE from poder_personaje 
        where id = {id_poder_seleccionado}"""
        self.cursor.execute(QueryEliminarPoder)
        print("Se ha eliminado exitosamente")
...
def MenuCRUDElementos (GM, Opcion):
    if Opcion == 1:
...
def MenuFichasPersonaje(GM, Opcion):
    if Opcion == 1:
        Eleccion = int(input("""¿Qué accion desea aplicar sobre las habilidades una ficha?
                             1.- Agregar una habilidad al personaje
                             2.- cambiar una habilidad de un personaje
                             3.- Eliminar una habilidad de un personaje \n"""))
        if Eleccion == 1:
            GM.AgregarHabilidadApersonaje()
        elif Eleccion == 2:
            GM.CambiarHabilidadPersonaje()
        elif Eleccion == 3:
            GM.eliminarhabilidadPersonaje()
            
    elif Opcion == 2:
        Eleccion = int(input("""¿Qué accion desea aplicar sobre los poderes de una ficha?
                        1.- Agregar un poder al personaje
                        2.- cambiar un poder de un personaje
                        3.- Eliminar un poder de un personaje \n"""))
        
        if Eleccion == 1:
            GM.AgregarPoderApersonaje()
        elif Eleccion == 2:
            GM.CambiarPoderPersonaje()
        elif Eleccion == 3:
            GM.eliminarPoderPersonaje()
        
    elif Opcion == 3:
        Eleccion = int(input("""¿Qué accion desea aplicar sobre los equipamientos de una ficha?
                1.- Agregar un equipamiento al personaje
                2.- Eliminar un equipamiento a un personaje \n"""))
        if Eleccion == 1:
            GM.AgregarEquipamientoPersonaje()
        elif Eleccion == 2:
            GM.EliminarEquipamientoPersonaje()
        
    elif Opcion == 4:
        print("Se procedera a cambiar el estado a un personaje")
        GM.cambiarEstadoPersonaje()
    elif Opcion == 5:
        print("Se procedera a aumentar el nivel de un personaje")
        GM.SubirNiveldePersonaje()